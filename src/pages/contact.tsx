      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-16 items-start">
          ...
          <div class="lg:col-span-2 space-y-6 fade-in">
            ...
            <h2 class="font-serif text-3xl font-bold text-brand-navy mb-2">Get in Touch</h2>
                ...
                const Inner = () => (
                  <div class="flex items-center gap-4">
                    <div class="w-11 h-11 bg-brand-navy rounded-xl flex items-center justify-center flex-shrink-0">
                      <i class={`fas ${c.icon} text-brand-gold text-sm`}></i>
                    </div>
                    <div>
                      <div class="text-xs text-gray-400 font-medium uppercase tracking-wide">{c.label}</div>
                      {c.lines.map((l, j) => <div key={j} class="text-brand-navy font-semibold text-sm">{l}</div>)}
                    </div>
                    {c.href && <i class="fas fa-chevron-right text-gray-300 text-xs ml-auto"></i>}
                  </div>
                )
                return c.href ? (
                  <a key={i} href={c.href} target={c.external ? '_blank' : undefined}
            ...
            <div class="bg-white rounded-2xl p-5 shadow-sm">
              <p class="text-sm font-semibold text-brand-navy mb-3">Follow Us</p>
              ...
              <form id="contact-page-form" class="space-y-5">
                ...
                <div class="flex items-start gap-3">
                  ...
                  <label for="cp-prayer" class="text-sm text-gray-600 cursor-pointer">
                    This is a <strong>prayer request</strong> — I'd like the prayer team to pray for me.
                  </label>
                </div>
                <button type="submit" id="cp-submit"
                  class="w-full bg-brand-navy hover:bg-brand-dark text-white font-semibold py-3.5 rounded-full transition-colors shadow-md">
